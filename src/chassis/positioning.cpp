...
void OrthogonalOdometry::update() {
    ...
    if (fabs(right_rot_dist_ - last_right_rot_dist_) < 0.005)
        right_rot_dist_ = last_right_rot_dist_;
    ...
    // Position updates
    const Vector d_pos = [&]() {
        const double d_l_dist = left_rot_dist_ - last_left_rot_dist_;
        const double d_r_dist = right_rot_dist_ - last_right_rot_dist_;
        ...
        const double d_y = (d_r_dist * cos(hardware.odometer.left_encoder_angle) -
                           ...
                           hardware.odometer.den;
        return Vector(d_x, d_y).rotate((last_heading_unwrapped_ + heading_unwrapped_) /
                                       2.0);
    }();
